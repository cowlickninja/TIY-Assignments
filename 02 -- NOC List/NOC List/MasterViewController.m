...
- (void)viewDidLoad
{
    ...
    //
    self.navigationItem.title = @"NOC List";
    ...
    // NICK NOTE: This was in the instructions before I edited it. >> self.agents = nil;

    //
    // 5. Call the method loadNocList so the tableview will actually have objects to load into its cells.
    //
    // NICK NOTE: Learned that you should use a dot for properties and space for calling method
    [self loadNocList];
...
- (void)loadNocList
{
    ...
    for (NSDictionary *agentDictionary in agents) {
        ...
        Agent * theAgent = [Agent agentWithDictionary: agentDictionary];
        [self.agents addObject:theAgent];
    }
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    ...
    if ([[segue identifier] isEqualToString:@"segueAgentDetail"])
    {
        ...
        // NICK NOTE: Ben helped with this and I have no idea how we came to this.
